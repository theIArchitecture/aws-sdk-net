...
{
    /// <summary>
    /// Amazon AIOps endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AIOps service requests.
    /// Collects values for AIOpsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AIOpsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAIOpsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAIOpsConfig)requestContext.ClientConfig;
            var result = new AIOpsEndpointParameters();
