...
{
    /// <summary>
    /// Amazon AWSHealth endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AWSHealth service requests.
    /// Collects values for AWSHealthEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AWSHealthEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAWSHealthEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAWSHealthConfig)requestContext.ClientConfig;
            var result = new AWSHealthEndpointParameters();
