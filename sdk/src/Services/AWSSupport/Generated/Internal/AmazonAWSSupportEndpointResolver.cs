...
{
    /// <summary>
    /// Amazon AWSSupport endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AWSSupport service requests.
    /// Collects values for AWSSupportEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AWSSupportEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAWSSupportEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAWSSupportConfig)requestContext.ClientConfig;
            var result = new AWSSupportEndpointParameters();
