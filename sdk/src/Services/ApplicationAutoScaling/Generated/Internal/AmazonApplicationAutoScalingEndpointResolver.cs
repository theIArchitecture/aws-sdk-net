...
{
    /// <summary>
    /// Amazon ApplicationAutoScaling endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ApplicationAutoScaling service requests.
    /// Collects values for ApplicationAutoScalingEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ApplicationAutoScalingEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonApplicationAutoScalingEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonApplicationAutoScalingConfig)requestContext.ClientConfig;
            var result = new ApplicationAutoScalingEndpointParameters();
