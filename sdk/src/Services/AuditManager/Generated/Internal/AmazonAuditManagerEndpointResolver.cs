...
{
    /// <summary>
    /// Amazon AuditManager endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for AuditManager service requests.
    /// Collects values for AuditManagerEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses AuditManagerEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonAuditManagerEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonAuditManagerConfig)requestContext.ClientConfig;
            var result = new AuditManagerEndpointParameters();
