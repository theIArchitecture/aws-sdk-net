...
{
    /// <summary>
    /// Amazon BCMDashboards endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for BCMDashboards service requests.
    /// Collects values for BCMDashboardsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses BCMDashboardsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonBCMDashboardsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonBCMDashboardsConfig)requestContext.ClientConfig;
            var result = new BCMDashboardsEndpointParameters();
