...
{
    /// <summary>
    /// Amazon Budgets endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Budgets service requests.
    /// Collects values for BudgetsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses BudgetsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonBudgetsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonBudgetsConfig)requestContext.ClientConfig;
            var result = new BudgetsEndpointParameters();
