...
{
    /// <summary>
    /// Amazon CloudControlApi endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CloudControlApi service requests.
    /// Collects values for CloudControlApiEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CloudControlApiEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCloudControlApiEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCloudControlApiConfig)requestContext.ClientConfig;
            var result = new CloudControlApiEndpointParameters();
