...
{
    /// <summary>
    /// Amazon CloudDirectory endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CloudDirectory service requests.
    /// Collects values for CloudDirectoryEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CloudDirectoryEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCloudDirectoryEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCloudDirectoryConfig)requestContext.ClientConfig;
            var result = new CloudDirectoryEndpointParameters();
