...
{
    /// <summary>
    /// Amazon CloudFormation endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CloudFormation service requests.
    /// Collects values for CloudFormationEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CloudFormationEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCloudFormationEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCloudFormationConfig)requestContext.ClientConfig;
            var result = new CloudFormationEndpointParameters();
