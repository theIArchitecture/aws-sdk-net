...
{
    /// <summary>
    /// Amazon CloudHSMV2 endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CloudHSMV2 service requests.
    /// Collects values for CloudHSMV2EndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CloudHSMV2EndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCloudHSMV2EndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCloudHSMV2Config)requestContext.ClientConfig;
            var result = new CloudHSMV2EndpointParameters();
