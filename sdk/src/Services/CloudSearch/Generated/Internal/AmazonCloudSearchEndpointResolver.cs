...
{
    /// <summary>
    /// Amazon CloudSearch endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CloudSearch service requests.
    /// Collects values for CloudSearchEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CloudSearchEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCloudSearchEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCloudSearchConfig)requestContext.ClientConfig;
            var result = new CloudSearchEndpointParameters();
