...
{
    /// <summary>
    /// Amazon CodeArtifact endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CodeArtifact service requests.
    /// Collects values for CodeArtifactEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CodeArtifactEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCodeArtifactEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCodeArtifactConfig)requestContext.ClientConfig;
            var result = new CodeArtifactEndpointParameters();
