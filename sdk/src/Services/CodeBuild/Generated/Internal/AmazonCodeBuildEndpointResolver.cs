...
{
    /// <summary>
    /// Amazon CodeBuild endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CodeBuild service requests.
    /// Collects values for CodeBuildEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CodeBuildEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCodeBuildEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCodeBuildConfig)requestContext.ClientConfig;
            var result = new CodeBuildEndpointParameters();
