...
{
    /// <summary>
    /// Amazon CodeCommit endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CodeCommit service requests.
    /// Collects values for CodeCommitEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CodeCommitEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCodeCommitEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCodeCommitConfig)requestContext.ClientConfig;
            var result = new CodeCommitEndpointParameters();
