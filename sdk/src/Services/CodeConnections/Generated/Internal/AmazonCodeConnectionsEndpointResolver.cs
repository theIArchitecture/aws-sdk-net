...
{
    /// <summary>
    /// Amazon CodeConnections endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CodeConnections service requests.
    /// Collects values for CodeConnectionsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CodeConnectionsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCodeConnectionsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCodeConnectionsConfig)requestContext.ClientConfig;
            var result = new CodeConnectionsEndpointParameters();
