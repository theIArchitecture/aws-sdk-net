...
{
    /// <summary>
    /// Amazon CodeDeploy endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CodeDeploy service requests.
    /// Collects values for CodeDeployEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CodeDeployEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCodeDeployEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCodeDeployConfig)requestContext.ClientConfig;
            var result = new CodeDeployEndpointParameters();
