...
{
    /// <summary>
    /// Amazon CodePipeline endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CodePipeline service requests.
    /// Collects values for CodePipelineEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CodePipelineEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCodePipelineEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCodePipelineConfig)requestContext.ClientConfig;
            var result = new CodePipelineEndpointParameters();
