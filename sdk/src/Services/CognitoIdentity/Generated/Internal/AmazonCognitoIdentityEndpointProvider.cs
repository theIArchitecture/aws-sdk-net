...
{
    /// <summary>
    /// Amazon CognitoIdentity endpoint provider.
    /// Resolves endpoint for given set of CognitoIdentityEndpointParameters.
    /// Can throw AmazonClientException if endpoint resolution is unsuccessful.
    /// </summary>
    public class AmazonCognitoIdentityEndpointProvider : IEndpointProvider
    {
        /// <summary>
        /// Resolve endpoint for CognitoIdentityEndpointParameters
        /// </summary>
            ...
            if (IsSet(refs["Region"]))
            {
                if ((refs["PartitionResult"] = Partition((string)refs["Region"])) != null)
                {
                    ...
                    if (Equals(refs["UseFIPS"], true))
                    {
                        ...
                        {
                            return new Endpoint(Interpolate(@"https://cognito-identity-fips.{Region}.{PartitionResult#dnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                        }
                    ...
                    return new Endpoint(Interpolate(@"https://cognito-identity.{Region}.{PartitionResult#dnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                }
            }
