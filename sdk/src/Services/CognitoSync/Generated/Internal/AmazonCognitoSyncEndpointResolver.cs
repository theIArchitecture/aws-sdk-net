...
{
    /// <summary>
    /// Amazon CognitoSync endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CognitoSync service requests.
    /// Collects values for CognitoSyncEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CognitoSyncEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCognitoSyncEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCognitoSyncConfig)requestContext.ClientConfig;
            var result = new CognitoSyncEndpointParameters();
