...
{
    /// <summary>
    /// Amazon CustomerProfiles endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for CustomerProfiles service requests.
    /// Collects values for CustomerProfilesEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses CustomerProfilesEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonCustomerProfilesEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonCustomerProfilesConfig)requestContext.ClientConfig;
            var result = new CustomerProfilesEndpointParameters();
