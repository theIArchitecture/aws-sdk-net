...
{
    /// <summary>
    /// Amazon DAX endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DAX service requests.
    /// Collects values for DAXEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DAXEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDAXEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDAXConfig)requestContext.ClientConfig;
            var result = new DAXEndpointParameters();
