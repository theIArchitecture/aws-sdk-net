...
{
    /// <summary>
    /// Amazon DLM endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DLM service requests.
    /// Collects values for DLMEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DLMEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDLMEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDLMConfig)requestContext.ClientConfig;
            var result = new DLMEndpointParameters();
