...
{
    /// <summary>
    /// Amazon DataExchange endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DataExchange service requests.
    /// Collects values for DataExchangeEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DataExchangeEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDataExchangeEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDataExchangeConfig)requestContext.ClientConfig;
            var result = new DataExchangeEndpointParameters();
