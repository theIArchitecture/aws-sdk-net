...
{
    /// <summary>
    /// Amazon DevOpsGuru endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DevOpsGuru service requests.
    /// Collects values for DevOpsGuruEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DevOpsGuruEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDevOpsGuruEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDevOpsGuruConfig)requestContext.ClientConfig;
            var result = new DevOpsGuruEndpointParameters();
