...
{
    /// <summary>
    /// Amazon DirectConnect endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DirectConnect service requests.
    /// Collects values for DirectConnectEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DirectConnectEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDirectConnectEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDirectConnectConfig)requestContext.ClientConfig;
            var result = new DirectConnectEndpointParameters();
