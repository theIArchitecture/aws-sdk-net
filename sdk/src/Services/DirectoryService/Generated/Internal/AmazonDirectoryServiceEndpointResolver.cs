...
{
    /// <summary>
    /// Amazon DirectoryService endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for DirectoryService service requests.
    /// Collects values for DirectoryServiceEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses DirectoryServiceEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonDirectoryServiceEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonDirectoryServiceConfig)requestContext.ClientConfig;
            var result = new DirectoryServiceEndpointParameters();
