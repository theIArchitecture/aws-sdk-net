...
{
    /// <summary>
    /// Amazon EMRContainers endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for EMRContainers service requests.
    /// Collects values for EMRContainersEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses EMRContainersEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonEMRContainersEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonEMRContainersConfig)requestContext.ClientConfig;
            var result = new EMRContainersEndpointParameters();
