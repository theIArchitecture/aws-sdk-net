...
{
    /// <summary>
    /// Amazon ElastiCache endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ElastiCache service requests.
    /// Collects values for ElastiCacheEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ElastiCacheEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonElastiCacheEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonElastiCacheConfig)requestContext.ClientConfig;
            var result = new ElastiCacheEndpointParameters();
