...
{
    /// <summary>
    /// Amazon ElasticBeanstalk endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ElasticBeanstalk service requests.
    /// Collects values for ElasticBeanstalkEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ElasticBeanstalkEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonElasticBeanstalkEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonElasticBeanstalkConfig)requestContext.ClientConfig;
            var result = new ElasticBeanstalkEndpointParameters();
