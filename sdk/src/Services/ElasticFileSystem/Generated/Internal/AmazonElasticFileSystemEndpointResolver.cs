...
{
    /// <summary>
    /// Amazon ElasticFileSystem endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ElasticFileSystem service requests.
    /// Collects values for ElasticFileSystemEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ElasticFileSystemEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonElasticFileSystemEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonElasticFileSystemConfig)requestContext.ClientConfig;
            var result = new ElasticFileSystemEndpointParameters();
