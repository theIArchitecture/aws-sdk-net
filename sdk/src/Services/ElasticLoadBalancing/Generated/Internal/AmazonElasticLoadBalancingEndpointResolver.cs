...
{
    /// <summary>
    /// Amazon ElasticLoadBalancing endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ElasticLoadBalancing service requests.
    /// Collects values for ElasticLoadBalancingEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ElasticLoadBalancingEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonElasticLoadBalancingEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonElasticLoadBalancingConfig)requestContext.ClientConfig;
            var result = new ElasticLoadBalancingEndpointParameters();
