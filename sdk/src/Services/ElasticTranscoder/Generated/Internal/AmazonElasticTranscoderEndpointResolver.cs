...
{
    /// <summary>
    /// Amazon ElasticTranscoder endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ElasticTranscoder service requests.
    /// Collects values for ElasticTranscoderEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ElasticTranscoderEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonElasticTranscoderEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonElasticTranscoderConfig)requestContext.ClientConfig;
            var result = new ElasticTranscoderEndpointParameters();
