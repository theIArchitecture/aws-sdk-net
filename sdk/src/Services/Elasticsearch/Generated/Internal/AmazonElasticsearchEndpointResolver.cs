...
{
    /// <summary>
    /// Amazon Elasticsearch endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Elasticsearch service requests.
    /// Collects values for ElasticsearchEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ElasticsearchEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonElasticsearchEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonElasticsearchConfig)requestContext.ClientConfig;
            var result = new ElasticsearchEndpointParameters();
