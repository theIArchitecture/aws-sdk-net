...
{
    /// <summary>
    /// Amazon EventBridge endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for EventBridge service requests.
    /// Collects values for EventBridgeEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses EventBridgeEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonEventBridgeEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonEventBridgeConfig)requestContext.ClientConfig;
            var result = new EventBridgeEndpointParameters();
