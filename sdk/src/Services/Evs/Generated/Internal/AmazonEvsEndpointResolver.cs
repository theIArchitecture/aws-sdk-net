...
{
    /// <summary>
    /// Amazon Evs endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Evs service requests.
    /// Collects values for EvsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses EvsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonEvsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonEvsConfig)requestContext.ClientConfig;
            var result = new EvsEndpointParameters();
