...
{
    /// <summary>
    /// Amazon FraudDetector endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for FraudDetector service requests.
    /// Collects values for FraudDetectorEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses FraudDetectorEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonFraudDetectorEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonFraudDetectorConfig)requestContext.ClientConfig;
            var result = new FraudDetectorEndpointParameters();
