...
namespace Amazon.GameLift.Model.Internal.MarshallTransformations
{
    ...
    public class DeploymentConfigurationUnmarshaller : IJsonUnmarshaller<DeploymentConfiguration, JsonUnmarshallerContext>
    {
        ...
        public DeploymentConfiguration Unmarshall(JsonUnmarshallerContext context, ref StreamingUtf8JsonReader reader)
        {
            ...
            {
                if (context.TestExpression("ImpairmentStrategy", targetDepth))
                {
                    var unmarshaller = StringUnmarshaller.Instance;
                    unmarshalledObject.ImpairmentStrategy = unmarshaller.Unmarshall(context, ref reader);
                    continue;
                }
                if (context.TestExpression("MinimumHealthyPercentage", targetDepth))
                {
                    var unmarshaller = NullableIntUnmarshaller.Instance;
                    unmarshalledObject.MinimumHealthyPercentage = unmarshaller.Unmarshall(context, ref reader);
                    continue;
                }
                if (context.TestExpression("ProtectionStrategy", targetDepth))
                {
                    var unmarshaller = StringUnmarshaller.Instance;
                    unmarshalledObject.ProtectionStrategy = unmarshaller.Unmarshall(context, ref reader);
                    continue;
                }
            }
