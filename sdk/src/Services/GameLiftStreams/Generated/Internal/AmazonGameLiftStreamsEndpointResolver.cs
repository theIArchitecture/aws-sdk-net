...
namespace Amazon.GameLiftStreams.Internal
{
    /// <summary>
    /// Amazon GameLiftStreams endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for GameLiftStreams service requests.
    /// Collects values for GameLiftStreamsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses GameLiftStreamsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGameLiftStreamsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGameLiftStreamsConfig)requestContext.ClientConfig;
            var result = new GameLiftStreamsEndpointParameters();
