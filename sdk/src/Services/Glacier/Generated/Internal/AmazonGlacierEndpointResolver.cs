...
{
    /// <summary>
    /// Amazon Glacier endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Glacier service requests.
    /// Collects values for GlacierEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses GlacierEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGlacierEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGlacierConfig)requestContext.ClientConfig;
            var result = new GlacierEndpointParameters();
