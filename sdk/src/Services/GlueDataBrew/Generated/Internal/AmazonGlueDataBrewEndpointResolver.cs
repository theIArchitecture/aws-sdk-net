...
{
    /// <summary>
    /// Amazon GlueDataBrew endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for GlueDataBrew service requests.
    /// Collects values for GlueDataBrewEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses GlueDataBrewEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGlueDataBrewEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGlueDataBrewConfig)requestContext.ClientConfig;
            var result = new GlueDataBrewEndpointParameters();
