...
{
    /// <summary>
    /// Amazon Greengrass endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Greengrass service requests.
    /// Collects values for GreengrassEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses GreengrassEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGreengrassEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGreengrassConfig)requestContext.ClientConfig;
            var result = new GreengrassEndpointParameters();
