...
{
    /// <summary>
    /// Amazon GreengrassV2 endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for GreengrassV2 service requests.
    /// Collects values for GreengrassV2EndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses GreengrassV2EndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGreengrassV2EndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGreengrassV2Config)requestContext.ClientConfig;
            var result = new GreengrassV2EndpointParameters();
