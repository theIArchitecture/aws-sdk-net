...
{
    /// <summary>
    /// Amazon GroundStation endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for GroundStation service requests.
    /// Collects values for GroundStationEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses GroundStationEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonGroundStationEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonGroundStationConfig)requestContext.ClientConfig;
            var result = new GroundStationEndpointParameters();
