...
{
    /// <summary>
    /// Amazon Inspector2 endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Inspector2 service requests.
    /// Collects values for Inspector2EndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses Inspector2EndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonInspector2EndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonInspector2Config)requestContext.ClientConfig;
            var result = new Inspector2EndpointParameters();
