...
{
    /// <summary>
    /// Amazon IoTEvents endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for IoTEvents service requests.
    /// Collects values for IoTEventsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses IoTEventsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonIoTEventsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonIoTEventsConfig)requestContext.ClientConfig;
            var result = new IoTEventsEndpointParameters();
