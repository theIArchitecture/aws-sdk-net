...
namespace Amazon.KeyspacesStreams.Internal
{
    /// <summary>
    /// Amazon KeyspacesStreams endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for KeyspacesStreams service requests.
    /// Collects values for KeyspacesStreamsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses KeyspacesStreamsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKeyspacesStreamsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKeyspacesStreamsConfig)requestContext.ClientConfig;
            var result = new KeyspacesStreamsEndpointParameters();
