...
{
    /// <summary>
    /// Amazon KinesisAnalytics endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for KinesisAnalytics service requests.
    /// Collects values for KinesisAnalyticsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses KinesisAnalyticsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKinesisAnalyticsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKinesisAnalyticsConfig)requestContext.ClientConfig;
            var result = new KinesisAnalyticsEndpointParameters();
