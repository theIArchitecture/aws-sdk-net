...
using Amazon.Util;
using Amazon.KinesisVideoWebRTCStorage.Endpoints;
...
{
    /// <summary>
    /// Amazon KinesisVideoWebRTCStorage endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for KinesisVideoWebRTCStorage service requests.
    /// Collects values for KinesisVideoWebRTCStorageEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses KinesisVideoWebRTCStorageEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonKinesisVideoWebRTCStorageEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonKinesisVideoWebRTCStorageConfig)requestContext.ClientConfig;
            var result = new KinesisVideoWebRTCStorageEndpointParameters();
