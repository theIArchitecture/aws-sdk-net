...
{
    /// <summary>
    /// Amazon Lightsail endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Lightsail service requests.
    /// Collects values for LightsailEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses LightsailEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonLightsailEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonLightsailConfig)requestContext.ClientConfig;
            var result = new LightsailEndpointParameters();
