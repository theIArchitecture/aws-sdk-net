...
{
    /// <summary>
    /// Amazon LookoutMetrics endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for LookoutMetrics service requests.
    /// Collects values for LookoutMetricsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses LookoutMetricsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonLookoutMetricsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonLookoutMetricsConfig)requestContext.ClientConfig;
            var result = new LookoutMetricsEndpointParameters();
