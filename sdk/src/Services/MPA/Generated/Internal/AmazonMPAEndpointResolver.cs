...
namespace Amazon.MPA.Internal
{
    /// <summary>
    /// Amazon MPA endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MPA service requests.
    /// Collects values for MPAEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MPAEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMPAEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMPAConfig)requestContext.ClientConfig;
            var result = new MPAEndpointParameters();
