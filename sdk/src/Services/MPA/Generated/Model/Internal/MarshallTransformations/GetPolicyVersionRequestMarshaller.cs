...
namespace Amazon.MPA.Model.Internal.MarshallTransformations
{
    ...
    public class GetPolicyVersionRequestMarshaller : IMarshaller<IRequest, GetPolicyVersionRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(GetPolicyVersionRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetPolicyVersionArn())
                throw new AmazonMPAException("Request object does not have required field PolicyVersionArn set");
            request.AddPathResource("{PolicyVersionArn}", StringUtils.FromString(publicRequest.PolicyVersionArn));
            request.ResourcePath = "/policy-versions/{PolicyVersionArn}";
