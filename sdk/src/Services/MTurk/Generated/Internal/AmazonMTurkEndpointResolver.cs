...
{
    /// <summary>
    /// Amazon MTurk endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MTurk service requests.
    /// Collects values for MTurkEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MTurkEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMTurkEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMTurkConfig)requestContext.ClientConfig;
            var result = new MTurkEndpointParameters();
