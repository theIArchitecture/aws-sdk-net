...
{
    /// <summary>
    /// Amazon MediaPackage endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MediaPackage service requests.
    /// Collects values for MediaPackageEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MediaPackageEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMediaPackageEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMediaPackageConfig)requestContext.ClientConfig;
            var result = new MediaPackageEndpointParameters();
