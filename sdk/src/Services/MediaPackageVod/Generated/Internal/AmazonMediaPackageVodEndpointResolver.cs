...
{
    /// <summary>
    /// Amazon MediaPackageVod endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MediaPackageVod service requests.
    /// Collects values for MediaPackageVodEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MediaPackageVodEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMediaPackageVodEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMediaPackageVodConfig)requestContext.ClientConfig;
            var result = new MediaPackageVodEndpointParameters();
