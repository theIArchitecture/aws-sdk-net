...
{
    /// <summary>
    /// Amazon MediaStoreData endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MediaStoreData service requests.
    /// Collects values for MediaStoreDataEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MediaStoreDataEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMediaStoreDataEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMediaStoreDataConfig)requestContext.ClientConfig;
            var result = new MediaStoreDataEndpointParameters();
