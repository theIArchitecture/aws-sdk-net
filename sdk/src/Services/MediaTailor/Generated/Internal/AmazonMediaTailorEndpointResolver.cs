...
{
    /// <summary>
    /// Amazon MediaTailor endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MediaTailor service requests.
    /// Collects values for MediaTailorEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MediaTailorEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMediaTailorEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMediaTailorConfig)requestContext.ClientConfig;
            var result = new MediaTailorEndpointParameters();
