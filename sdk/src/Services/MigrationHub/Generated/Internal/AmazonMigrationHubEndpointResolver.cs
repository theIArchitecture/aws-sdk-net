...
{
    /// <summary>
    /// Amazon MigrationHub endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for MigrationHub service requests.
    /// Collects values for MigrationHubEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses MigrationHubEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonMigrationHubEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonMigrationHubConfig)requestContext.ClientConfig;
            var result = new MigrationHubEndpointParameters();
