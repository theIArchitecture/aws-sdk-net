...
{
    /// <summary>
    /// Amazon Neptune endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Neptune service requests.
    /// Collects values for NeptuneEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses NeptuneEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonNeptuneEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonNeptuneConfig)requestContext.ClientConfig;
            var result = new NeptuneEndpointParameters();
