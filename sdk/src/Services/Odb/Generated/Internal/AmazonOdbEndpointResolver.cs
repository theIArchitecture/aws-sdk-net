...
{
    /// <summary>
    /// Amazon Odb endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Odb service requests.
    /// Collects values for OdbEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses OdbEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonOdbEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonOdbConfig)requestContext.ClientConfig;
            var result = new OdbEndpointParameters();
