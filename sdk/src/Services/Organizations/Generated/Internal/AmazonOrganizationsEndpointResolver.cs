...
{
    /// <summary>
    /// Amazon Organizations endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Organizations service requests.
    /// Collects values for OrganizationsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses OrganizationsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonOrganizationsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonOrganizationsConfig)requestContext.ClientConfig;
            var result = new OrganizationsEndpointParameters();
