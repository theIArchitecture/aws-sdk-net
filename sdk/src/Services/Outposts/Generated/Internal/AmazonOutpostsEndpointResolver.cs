...
{
    /// <summary>
    /// Amazon Outposts endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Outposts service requests.
    /// Collects values for OutpostsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses OutpostsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonOutpostsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonOutpostsConfig)requestContext.ClientConfig;
            var result = new OutpostsEndpointParameters();
