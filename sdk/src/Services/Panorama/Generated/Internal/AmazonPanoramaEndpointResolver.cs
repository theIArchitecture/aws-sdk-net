...
{
    /// <summary>
    /// Amazon Panorama endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Panorama service requests.
    /// Collects values for PanoramaEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses PanoramaEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonPanoramaEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonPanoramaConfig)requestContext.ClientConfig;
            var result = new PanoramaEndpointParameters();
