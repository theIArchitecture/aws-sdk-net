...
{
    /// <summary>
    /// Amazon Personalize endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Personalize service requests.
    /// Collects values for PersonalizeEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses PersonalizeEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonPersonalizeEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonPersonalizeConfig)requestContext.ClientConfig;
            var result = new PersonalizeEndpointParameters();
