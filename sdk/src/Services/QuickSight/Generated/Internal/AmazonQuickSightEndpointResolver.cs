...
{
    /// <summary>
    /// Amazon QuickSight endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for QuickSight service requests.
    /// Collects values for QuickSightEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses QuickSightEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonQuickSightEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonQuickSightConfig)requestContext.ClientConfig;
            var result = new QuickSightEndpointParameters();
