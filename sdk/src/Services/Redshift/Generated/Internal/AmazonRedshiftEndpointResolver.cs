...
{
    /// <summary>
    /// Amazon Redshift endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Redshift service requests.
    /// Collects values for RedshiftEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses RedshiftEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonRedshiftEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonRedshiftConfig)requestContext.ClientConfig;
            var result = new RedshiftEndpointParameters();
