...
{
    /// <summary>
    /// Amazon ResourceGroups endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ResourceGroups service requests.
    /// Collects values for ResourceGroupsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ResourceGroupsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonResourceGroupsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonResourceGroupsConfig)requestContext.ClientConfig;
            var result = new ResourceGroupsEndpointParameters();
