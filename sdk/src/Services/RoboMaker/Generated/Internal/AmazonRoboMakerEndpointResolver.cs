...
{
    /// <summary>
    /// Amazon RoboMaker endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for RoboMaker service requests.
    /// Collects values for RoboMakerEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses RoboMakerEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonRoboMakerEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonRoboMakerConfig)requestContext.ClientConfig;
            var result = new RoboMakerEndpointParameters();
