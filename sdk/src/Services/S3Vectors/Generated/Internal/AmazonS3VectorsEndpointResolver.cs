...
{
    /// <summary>
    /// Amazon S3Vectors endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for S3Vectors service requests.
    /// Collects values for S3VectorsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses S3VectorsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonS3VectorsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonS3VectorsConfig)requestContext.ClientConfig;
            var result = new S3VectorsEndpointParameters();
