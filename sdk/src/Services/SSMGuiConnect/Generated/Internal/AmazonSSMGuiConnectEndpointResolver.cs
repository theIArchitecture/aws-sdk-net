...
{
    /// <summary>
    /// Amazon SSMGuiConnect endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SSMGuiConnect service requests.
    /// Collects values for SSMGuiConnectEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SSMGuiConnectEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSSMGuiConnectEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSSMGuiConnectConfig)requestContext.ClientConfig;
            var result = new SSMGuiConnectEndpointParameters();
