...
{
    /// <summary>
    /// Amazon SSO endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SSO service requests.
    /// Collects values for SSOEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SSOEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSSOEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSSOConfig)requestContext.ClientConfig;
            var result = new SSOEndpointParameters();
