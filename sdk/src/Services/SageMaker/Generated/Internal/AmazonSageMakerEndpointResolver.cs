...
{
    /// <summary>
    /// Amazon SageMaker endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SageMaker service requests.
    /// Collects values for SageMakerEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SageMakerEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSageMakerEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSageMakerConfig)requestContext.ClientConfig;
            var result = new SageMakerEndpointParameters();
