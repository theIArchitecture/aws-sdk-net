...
{
    /// <summary>
    /// Amazon SavingsPlans endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SavingsPlans service requests.
    /// Collects values for SavingsPlansEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SavingsPlansEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSavingsPlansEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSavingsPlansConfig)requestContext.ClientConfig;
            var result = new SavingsPlansEndpointParameters();
