...
{
    /// <summary>
    /// Amazon ServiceCatalog endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ServiceCatalog service requests.
    /// Collects values for ServiceCatalogEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ServiceCatalogEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonServiceCatalogEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonServiceCatalogConfig)requestContext.ClientConfig;
            var result = new ServiceCatalogEndpointParameters();
