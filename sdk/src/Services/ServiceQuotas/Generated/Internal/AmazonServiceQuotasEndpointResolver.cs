...
{
    /// <summary>
    /// Amazon ServiceQuotas endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for ServiceQuotas service requests.
    /// Collects values for ServiceQuotasEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses ServiceQuotasEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonServiceQuotasEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonServiceQuotasConfig)requestContext.ClientConfig;
            var result = new ServiceQuotasEndpointParameters();
