...
{
    /// <summary>
    /// Amazon SimpleDB endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SimpleDB service requests.
    /// Collects values for SimpleDBEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SimpleDBEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSimpleDBEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSimpleDBConfig)requestContext.ClientConfig;
            var result = new SimpleDBEndpointParameters();
