...
namespace Amazon.SimpleNotificationService.Internal
{
    ...
    /// </summary>
    public class AmazonSimpleNotificationServiceEndpointProvider : IEndpointProvider
    {
        /// <summary>
            ...
            if (IsSet(refs["Region"]))
            {
                if ((refs["PartitionResult"] = Partition((string)refs["Region"])) != null)
                {
                    if (Equals(refs["UseFIPS"], true) && Equals(refs["UseDualStack"], true))
                    {
                        ...
                        {
                            return new Endpoint(Interpolate(@"https://sns-fips.{Region}.{PartitionResult#dualStackDnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                        }
                    ...
                    if (Equals(refs["UseDualStack"], true))
                    {
                        ...
                        {
                            return new Endpoint(Interpolate(@"https://sns.{Region}.{PartitionResult#dualStackDnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                        }
                        throw new AmazonClientException("DualStack is enabled but this partition does not support DualStack");
                    }
                    return new Endpoint(Interpolate(@"https://sns.{Region}.{PartitionResult#dnsSuffix}", refs), InterpolateJson(@"", refs), InterpolateJson(@"", refs));
                }
            }
