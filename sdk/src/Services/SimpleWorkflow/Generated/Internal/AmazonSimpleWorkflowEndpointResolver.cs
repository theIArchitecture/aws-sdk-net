...
{
    /// <summary>
    /// Amazon SimpleWorkflow endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for SimpleWorkflow service requests.
    /// Collects values for SimpleWorkflowEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SimpleWorkflowEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSimpleWorkflowEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSimpleWorkflowConfig)requestContext.ClientConfig;
            var result = new SimpleWorkflowEndpointParameters();
