...
{
    /// <summary>
    /// Amazon Snowball endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Snowball service requests.
    /// Collects values for SnowballEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SnowballEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSnowballEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSnowballConfig)requestContext.ClientConfig;
            var result = new SnowballEndpointParameters();
