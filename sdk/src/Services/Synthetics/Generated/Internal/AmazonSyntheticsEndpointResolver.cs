...
{
    /// <summary>
    /// Amazon Synthetics endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for Synthetics service requests.
    /// Collects values for SyntheticsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses SyntheticsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonSyntheticsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonSyntheticsConfig)requestContext.ClientConfig;
            var result = new SyntheticsEndpointParameters();
