...
{
    /// <summary>
    /// Amazon VoiceID endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for VoiceID service requests.
    /// Collects values for VoiceIDEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses VoiceIDEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonVoiceIDEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonVoiceIDConfig)requestContext.ClientConfig;
            var result = new VoiceIDEndpointParameters();
