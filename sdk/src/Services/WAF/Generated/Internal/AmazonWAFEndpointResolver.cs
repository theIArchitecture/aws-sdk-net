...
{
    /// <summary>
    /// Amazon WAF endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for WAF service requests.
    /// Collects values for WAFEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses WAFEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonWAFEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonWAFConfig)requestContext.ClientConfig;
            var result = new WAFEndpointParameters();
