...
{
    /// <summary>
    /// Amazon WorkDocs endpoint resolver.
    /// Custom PipelineHandler responsible for resolving endpoint and setting authentication parameters for WorkDocs service requests.
    /// Collects values for WorkDocsEndpointParameters and then tries to resolve endpoint by calling 
    /// ResolveEndpoint method on GlobalEndpoints.Provider if present, otherwise uses WorkDocsEndpointProvider.
    /// Responsible for setting authentication and http headers provided by resolved endpoint.
    /// </summary>
    public class AmazonWorkDocsEndpointResolver : BaseEndpointResolver
    {
        ...
        {
            var config = (AmazonWorkDocsConfig)requestContext.ClientConfig;
            var result = new WorkDocsEndpointParameters();
