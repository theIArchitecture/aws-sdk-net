    ...
    public class HttpRequestWithLabelsRequestMarshaller : IMarshaller<IRequest, HttpRequestWithLabelsRequest> , IMarshaller<IRequest,AmazonWebServiceRequest>
    {
        ...
        public IRequest Marshall(HttpRequestWithLabelsRequest publicRequest)
        {
            ...
            if (!publicRequest.IsSetBoolean())
                throw new AmazonRestJsonProtocolException("Request object does not have required field Boolean set");
            ...
            if (!publicRequest.IsSetInteger())
                throw new AmazonRestJsonProtocolException("Request object does not have required field Integer set");
            request.AddPathResource("{integer}", StringUtils.FromInt(publicRequest.Integer));
            if (!publicRequest.IsSetLong())
                throw new AmazonRestJsonProtocolException("Request object does not have required field Long set");
            request.AddPathResource("{long}", StringUtils.FromLong(publicRequest.Long));
            if (!publicRequest.IsSetShort())
                throw new AmazonRestJsonProtocolException("Request object does not have required field Short set");
            request.AddPathResource("{short}", StringUtils.FromInt(publicRequest.Short));
            if (!publicRequest.IsSetString())
                throw new AmazonRestJsonProtocolException("Request object does not have required field String set");
            request.AddPathResource("{string}", StringUtils.FromString(publicRequest.String));
            if (!publicRequest.IsSetTimestamp())
                throw new AmazonRestJsonProtocolException("Request object does not have required field Timestamp set");
